...
with tempfile.TemporaryDirectory(prefix='meilleur-auto-media-test-') as temp:
    ...
    for lang in ['fr','en']:
        for i in range(1,14):
            (t/f'content/{lang}/blog/controle-{i:02}.md').write_text('---\n'+ '\n'.join(k+': '+json.dumps(v,ensure_ascii=False) for k,v in dict(title=f'Article de contrôle {i:02}',description='Fixture éditoriale hors publication.',date=f'2026-08-{i:02}',lastmod=f'2026-08-{i:02}',author='redaction-meilleur-choix-auto',translationKey=f'controle-{i:02}',categories=['Modeles et comparatifs'],image='images/route-alpine.jpg',imageAlt='Photo de contrôle').items())+'\n---\n\n## Section de contrôle\n\nTexte de test hors publication.\n')
